...
type PreviewDomain = { id: string; label: string; status: PreviewStatus; metrics: readonly ComparisonRow[]; sessionRows: readonly ComparisonRow[]; historyCount: number; recentCount: number; changeReason: string; thinkingChange: string; nextCheck: string; pastUtterance: string; recentUtterance: string };
...
const previewDomains: Record<PreviewMode, readonly PreviewDomain[]> = {
  ...
  LIFE: [
    ...
    { id: "change-receive", label: "거스름돈 받기", status: "growing", metrics: [["정답률", "45%", "68%"], ["정답까지 평균", "3.0회", "2.0회"], ["혼자 말하기", "10%", "30%"]], sessionRows: [["실생활 정답률", "45%", "68%"], ["정답까지 평균", "3.0회", "2.0회"], ["혼자 말하기", "10%", "30%"], ["발화 단계 사용 비율 (L4/L3/L2/L1/L0)", "10/30/30/20/10%", "30/30/20/15/5%"]], historyCount: 8, recentCount: 3, changeReason: "최근에는 맞힌 비율이 높아졌지만, 계산 순서를 스스로 말하는 연습이 더 필요해요.", thinkingChange: "과거에는 답을 미뤘지만, 최근에는 낸 돈에서 가격을 빼는 방법을 말했어요.", nextCheck: "다음 활동에서는 계산 전후의 금액을 각각 말하는지 확인해 보세요.", pastUtterance: "잘 모르겠어요.", recentUtterance: "5,000원에서 가격을 빼요." },
    { id: "queue", label: "줄 서기", status: "collecting", metrics: [["정답률", "—", "85%"], ["정답까지 평균", "—", "1.5회"], ["혼자 말하기", "—", "40%"]], sessionRows: [["실생활 정답률", "—", "85%"], ["정답까지 평균", "—", "1.5회"], ["혼자 말하기", "—", "40%"], ["발화 단계 사용 비율 (L4/L3/L2/L1/L0)", "—", "40/30/20/10/0%"]], historyCount: 3, recentCount: 3, changeReason: "최근 기록은 좋지만, 과거와 비교할 만큼의 기록을 더 모으고 있어요.", thinkingChange: "최근에는 두 줄의 사람 수를 비교해 더 짧은 줄을 말했어요.", nextCheck: "다음 활동에서는 두 수를 어떤 방법으로 비교했는지 말하는지 확인해 보세요.", pastUtterance: "비교할 과거 발화 기록을 모으는 중이에요.", recentUtterance: "왼쪽 줄이 두 명이라 더 짧아요." },
  ],
};
  ...
  return <main className="report-page numeric-preview-page">
    ...
    <article className="report-paper numeric-preview" data-report-format="a4">
      ...
      <section className="numeric-preview__section" aria-labelledby="numeric-trend-title">
        ...
        <div className="numeric-session-comparison" aria-label={`${modeLabels[mode]} · ${selectedDomain.label} 과거 전체와 최근 비교`}><div className="numeric-session-comparison__head"><span></span><strong>과거 전체</strong><strong>최근</strong></div>{selectedDomain.sessionRows.map(([label, past, recent]) => <div key={label}><span>{label}</span><strong>{past}</strong><strong>{recent}</strong></div>)}</div>
